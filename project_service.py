...
def create_project(project: Project) -> int:
    """Saves a new project. Returns the new project's ID."""
    conn = get_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("""
...
def _row_to_project(row) -> Project:
    return Project(
        ...
        project_type=row["project_type"] if "project_type" in row.keys() else "BESS",
...
def project_display_name(project) -> str:
    """Returns display name with project type icon for dropdowns."""
    from models.models import PROJECT_TYPE_ICONS
    icon = PROJECT_TYPE_ICONS.get(getattr(project, 'project_type', 'BESS'), '📁')
    return f"{icon}  {project.name}"
